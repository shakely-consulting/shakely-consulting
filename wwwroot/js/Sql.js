(function() {
	...
	window.Sql = {
		...
		initGrid: function() {
            ...
            var url = "https://localhost:7239/btcusd.json";
        ...
        tableize: function () {
            new gridjs.Grid({
                ...
                columns: ['iBook', 'Book'],
                server: {
                    url: 'https://localhost:7239/btcusd.json',
                    handle: (res) => {
		...
		LoadTables: async function(tblName) {
			...
			if (tblcnt == el.length && el.length > 0) {
				SC.logit("table count: " + tblcnt);
			}
			return await Sql.loadTableSelectable(tblName);
			// await DotNet.invokeMethodAsync('Shakely-Consulting', 'LoadTables', tblName);
		},
